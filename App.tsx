...
import { CsvIcon, PlusIcon, TrashIcon, ArrowRightIcon, DownloadIcon, RefreshIcon, EraserIcon } from './components/Icon';
import { EditableContactTable } from './components/EditableContactTable';
import { VcfPreviewModal } from './components/VcfPreviewModal';
import { SuccessModal } from './components/SuccessModal';
import { ClearColumnModal } from './components/ClearColumnModal';
...
const App: React.FC = () => {
  ...
  const handleBulkChange = useCallback((newContacts: Contact[]) => {
  ...
  const handleClearColumn = useCallback((key: keyof Contact) => {
    setContacts(prev => prev.map(contact => ({ ...contact, [key]: '' })));
    setIsConverted(false);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4 font-sans">
      <div className="w-full max-w-7xl mx-auto">
        ...
        <main className="bg-slate-800/50 border border-slate-700 rounded-xl shadow-2xl shadow-slate-950/50 p-6">
            <div className="flex items-center gap-4 mb-4">
                ...
                <button onClick={() => setShowClearColumnModal(true)} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-md font-semibold transition-colors flex items-center gap-2 text-slate-200">
                    <EraserIcon className="w-5 h-5" /> Clear Column
                </button>
            </div>
        ...
        <footer className="text-center mt-8 text-slate-400 text-sm space-y-2">
          <p className="flex items-center justify-center gap-2">
            ...
            <span className="text-slate-500">No Internet Required</span>
          </p>
          <p className="text-slate-500">
            ...
            {' '}initiative by{' '}
            <a 
              href="https://github.com/Deepakgowda007" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="text-sky-400 hover:text-sky-300 font-semibold transition-colors"
            >
      ...
      <ClearColumnModal
        ...
        onClose={() => setShowClearColumnModal(false)}
        onClearColumn={handleClearColumn}
      />
    </div>
  );
